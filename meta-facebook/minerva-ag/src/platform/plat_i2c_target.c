...
void i2c_bridge_command_handler(plat_msg_t *msg)
{
	...
	I2C_MSG i2c_msg = { 0 };
	uint8_t retry = 5;
    if (msg->write_len > sizeof(i2c_msg.data)) {
        LOG_ERR("write_len (%d) exceeds I2C_MSG buffer (%zu)", 
                msg->write_len, sizeof(i2c_msg.data));
...
void plat_command_msgq_handler()
{
	...
	while (1) {
		...
		case I2C_BRIDGE_COMMAND_REG: {
            ...
            if (rlen > sizeof(msg.rdata)) {
                LOG_ERR("I2C rlen (%u) exceeds buffer limit (%zu)", rlen, sizeof(msg.rdata));
                break;
            }
